#!/usr/bin/env python3
"""Ensemble 5-modèles (3×M5 + M11 + M18) + TTA → DÉPASSER SOTA 96%.

5 modèles, 3 architectures différentes (M5 simple, M11 1-ResBlock, M18 4-ResBlocks)
→ diversité maximale → gain ensemble maximal → cible 96%+.
"""
...
def main():
    ...
    print(f"\n[ENSEMBLE 5-arch] {len(models)} modèles × {len(TTA_RATES)} TTA",flush=True)
    ...
    print(f"\n{'='*60}")
    print(f"ENSEMBLE 5-ARCHITECTURES + TTA — test officiel FINAL")
    print(f"{'='*60}")
    ...
    json.dump({"ensemble_tta_acc":acc_tta,"ensemble_acc":acc_ens,"n_models":len(models),
              ...
              open("ocm26400/audio_ensemble5_tta_results.json","w"),indent=2)
    print("  [sauvé] ocm26400/audio_ensemble5_tta_results.json")
