...
def make_batch(words, rule, training=True):
    ...
    for i, w in enumerate(words):
        ...
        if training:
            p = random.uniform(0.15, 1.0)                      # taux PAR SÉQUENCE (rapports/08)
            for j in range(W_A):
                if a[j] != VOC["_"] and random.random() < p:
                    seq[i, W_Q + j] = VOC["_"]                # SOUS-ENSEMBLE masqué
                else:
                    seq[i, W_Q + j] = a[j]                    # visible (vérité)
        else:
...
def main():
    print("="*64); print("PHASE 1 v2 (raffinée) — grok morphologique → cible 0.927"); print("="*64)
    ...
    import json
    json.dump(results, open("ocm26400/phase1_v2_results.json", "w"), indent=2, default=str)
    print("[sauvé]")
