...
5. ⏳ **P4 Pont v6→AMV** — prémises : (1) P2 V>64 ✅, (2) partition meta ✅, (3) **accuracy single-forward v6 MESURÉE ✅** (`spxlm_v6/measure_single_forward.py` : single 96.4% ≥ diffuse 91.3% sur eval verify_v6 — réfute l'estimation DA ~50-60% ; le raffinement itératif n'achète rien, il dégrade légèrement). Reste à traiter : (4) L_step non-différentiable (`acsp.py:38`), alignement hidden 256-d → partitions AMV, dépendance dico réel (easée par P2). Caveat honnête : eval verify_v6 (1 champ masqué) plus facile que l'eval entraînement 0.695 — la comparaison within-eval (single ≥ diffuse) est la conclusion valide.
